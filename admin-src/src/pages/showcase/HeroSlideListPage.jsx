...
import { deleteHeroSlide, getHeroSlides, updateHeroSlide } from "../../services/showcase.api";

function HeroSlideListPage() {
  ...
  const fetchData = async (page = 1, limit = 20, search = "") => {
    setLoading(true);
    try {
      const result = await getHeroSlides({ page, limit, search });
      ...
      notifyError(error?.response?.data?.error?.message || "Cannot load hero slides");
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = useMemo(
    () => [
      ...
      { title: "Updated", dataIndex: "updated_at", width: 170, render: (value) => dayjs(value).format("YYYY-MM-DD HH:mm") },
      {
        ...
        render: (_, row) => (
          <Space>
            <Button onClick={() => navigate(`/showcase/hero-slides/${row.id}`)}>Edit</Button>
            <Popconfirm
              title="Delete item?"
              okText="Delete"
  ...
  return (
    <div style={{ height: "calc(100vh - 96px)", display: "flex", flexDirection: "column", minHeight: 0 }}>
      <PageHeader
        title="Hero Slides"
        extra={<Button type="primary" onClick={() => navigate("/showcase/hero-slides/new")}>Add Project</Button>}
      />
