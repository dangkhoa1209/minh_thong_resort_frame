...
window.cardsEffect.ScrollObserver = ScrollObserver;
...
class ContainerScrollObserver extends ScrollObserver {
  ...
  _onScroll() {
    ...
    const percentageY = window.cardsEffect.clamp(currentScrollY / totalScrollY, 0, 1) || 0

    const currentScrollX = this._container.scrollLeft
    const totalScrollX =
      this._container.scrollWidth - this._container.clientWidth
    const percentageX = window.cardsEffect.clamp(currentScrollX / totalScrollX, 0, 1) || 0
...
class ElementScrollObserver extends ScrollObserver {
  ...
  _calculatePercentageY() {
    ...
    return window.cardsEffect.clamp((startPoint - rect.top) / viewHeight, 0, 1)
  }
  ...
  _calculatePercentageX() {
    ...
    return window.cardsEffect.clamp((startPoint - rect.left) / viewWidth, 0, 1)
  }
